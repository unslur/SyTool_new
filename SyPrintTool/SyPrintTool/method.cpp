...
string GetNumbers(const char* orgID, const char* taskID, int num, const char* dbPathName) {
	...
	int rc;
	rc = sqlite3_open(dbPathName, &db);
	//rc=sqlite3_open_v2(dbPathName,&db,SQLITE_OPEN_READWRITE | SQLITE_OPEN_FULLMUTEX,NULL);
	//Sleep(1000);
	sprintf(buff, "select box_sourcecode, flag from number where printtask_code='%s' limit %d", taskID, num);
...
string method::GetNumber(LPCTSTR inVar)
{
	...
	//MessageBox(NULL,rtnData.c_str(), NULL, MB_OK);
	return rtnData;
}
