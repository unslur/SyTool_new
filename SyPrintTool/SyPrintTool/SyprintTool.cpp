...
string function(vector<string> RecvStr) {
	...
	{
		//MessageBox(NULL,RecvStr[2].c_str(), NULL, MB_OK);
...
string debugstring(string message){
	//string a="";
	string b=",\"value\":\"";//26
	string c="sendMessagesstring messages";
	message=message.substr(0, message.find_last_of('}'));
	//MessageBox(NULL, message.c_str(), NULL, MB_YESNO);
...
bool sendMessage(string message) {
	...
	if (lentmp>2556&&lentmp<2822)
	{
		message=debugstring(message);
	}
	//message=debugstring();
	...
	//message+="11";
	//MessageBox(NULL,ss, NULL, MB_OK);
	char *bOutLen = reinterpret_cast<char *>(&len);
